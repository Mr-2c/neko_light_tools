...
Standalone, CPU-only mesh utilities for Neko, in scientific Python.  One
shared library (`nekolight/`) holds every byte layout, topology table and
...
hard requirement (scipy for partitioning).
...
* `spectral` (default): recursive spectral bisection -- Nek5000 `genmap`'s
  algorithm with scipy eigensolvers (dense / shift-invert Lanczos /
...
confirm the result is correct for your own mesh -- the suite makes that
easy to do on a case you trust.
